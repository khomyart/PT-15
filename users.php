...
$usersList = $users->getList($searchValue);
...
if ($_POST['regButton']) {
    ...
    if ($users->add($regEmail, $regPassword, $regNickname)) {
        ...
        $feedbackUserError = $users->errorList($regEmail, $regPassword, $regNickname);
    }
}
...
if (isset($_POST['userEditSave'])) {
    ...
    if (($result['email'] == $email) && ($result['nickname'] == $nickname)) {
        if ($users->edit($id, $email, $nickname, $type, 'none')) {
            //Preventing re-registration by pressing F5 (sending post request again and again)
            header('Location: Users.php');
        } else {
            $feedbackUserError = $users->altErrorList($email, $nickname);
        }
    } elseif (($result['email'] == $email) && ($result['nickname'] !== $nickname)) {
        if ($users->edit($id, $email, $nickname, $type, 'nickname')) {
            header('Location: Users.php');
        } else {
            $feedbackUserError = $users->editNicknameExistenceChecker($nickname);
        }
    } elseif (($result['email'] !== $email) && ($result['nickname'] == $nickname)) {
        if ($users->edit($id, $email, $nickname, $type, 'login')) {
            header('Location: Users.php');
        } else {
            $feedbackUserError = $users->editLoginExistenceChecker($email);
        }
    } else {
        if ($users->edit($id, $email, $nickname, $type, 'full')) {
            header('Location: Users.php');
        } else {
            $feedbackUserError = $users->errorList($email, '******', $nickname);
        }
    }
}
...
if(isset($_POST['editPasswordConfirm'])) {
    ...
    if (empty($users->passwordChange($id, $oldPassword, $newPassword, $repeatNewPassword))) {
        header('Location: Users.php');
        die();
    } else {
        $feedbackUserError = $users->passwordChange($id, $oldPassword, $newPassword, $repeatNewPassword);
    }
}
